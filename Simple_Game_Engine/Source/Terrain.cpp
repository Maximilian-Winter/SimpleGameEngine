...
bool Terrain::Initialize(ID3D11Device* device, MaterialManager* MatManager, std::string filename)
{
	bool result;
	...
	TerrainFile.GetStringValueByName("terrain", "heightmap", heightMapFilename);
	TerrainFile.GetStringValueByName("terrain", "terrainMaterialFile", MaterialFilename);
	TerrainFile.GetStringValueByName("terrain", "terrainMaterial", m_MaterialName);

	result = MatManager->AddMaterialFile(device, MaterialFilename);
	if (!result)
	{
		return false;
	}
...
int Terrain::GetSubsetCount()
{
	return 1;
}

void Terrain::GetWidthHeightAndDepth(float & width, float & height, float & depth)
{
	width = m_AABB_width;
...
bool Terrain::LoadHeightMap(std::string filename)
{
	...
	if (m_terrainWidth <= 0 || m_terrainWidth >= 100000 || m_terrainHeight <= 0 || m_terrainHeight >= 100000)
	{
		return false;
	}
	...
	// Read the image data into the height map and find the min and max vertices for a aabb.
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			...
			minVertex.x = min(minVertex.x, m_heightMap[index].x);	// Find smallest x value in model
			minVertex.y = min(minVertex.y, m_heightMap[index].y);	// Find smallest y value in model
			minVertex.z = min(minVertex.z, m_heightMap[index].z);	// Find smallest z value in model

			//Get the largest vertex 
			maxVertex.x = max(maxVertex.x, m_heightMap[index].x);	// Find largest x value in model
			maxVertex.y = max(maxVertex.y, m_heightMap[index].y);	// Find largest y value in model
			maxVertex.z = max(maxVertex.z, m_heightMap[index].z);	// Find largest z value in model
...
VerticesRenderData Terrain::GetSubsetRenderData(MaterialManager* materialManager, int subsetIndex)
{
	VerticesRenderData TempData;

	TempData.drawAmount = m_indexCount;
	TempData.start = 0;
	TempData.Material = materialManager->GetMaterial(m_MaterialName);
